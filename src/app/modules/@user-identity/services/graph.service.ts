...
import { addClaimsToStorage, getClaimsFromStorage } from '../utils/storage-utils';
...
export class GraphService {
    ...
    handleClaimsChallenge(response: any, providerOptions: ProviderOptions): void {
        ...
        addClaimsToStorage(
            ...
            `cc.${environment.msalConfig.auth.clientId}.${account?.idTokenClaims?.oid}.${new URL(providerOptions.endpoint).hostname
            }`
            ...
            let claim =
                this.authService.instance.getActiveAccount()! &&
                    getClaimsFromStorage(
                        `cc.${environment.msalConfig.auth.clientId}.${this.authService.instance.getActiveAccount()?.idTokenClaims?.oid
                        }.${resource}`
                    )
                    ? window.atob(
                        getClaimsFromStorage(
                            `cc.${environment.msalConfig.auth.clientId}.${this.authService.instance.getActiveAccount()?.idTokenClaims
                                ?.oid
                            }.${resource}`
                        )
                    )
